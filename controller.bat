...
TITLE LMINA ENTERPRISE ORCHESTRATOR v3.0
COLOR 0B

echo.
echo  #####################################################################
echo  #                                                                   #
echo  #    LMINA STREAMING PLATFORM - ENTERPRISE ORCHESTRATOR             #
...
echo [SYSTEM] Starting LMINA services...
timeout /t 1 >nul
...
cd /d "%~dp0backend"
start /min "LMINA_API" cmd /c "run_api_robust.bat"
...
if exist "backend\bin\cloudflared.exe" (
    ...
    :: Start cloudflared
    start /min "LMINA_TUNNEL" cmd /c "backend\bin\cloudflared.exe tunnel --url http://localhost:8000 --no-autoupdate > backend\logs\tunnel.log 2>&1"
